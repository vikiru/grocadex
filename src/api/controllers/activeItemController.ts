...
export async function createActiveItems(
    req: Request,
    res: Response,
): Promise<void> {
    ...
    try {
        ...
        response['error'] =
            'There was an error saving active items to the database.';
        res.status(500).json(response);
    }
}

export async function deleteActiveItems(
    req: Request,
    res: Response,
): Promise<void> {
    const userId = req.user;
    ...
    try {
        ...
        logger.error(
            `Error removing active items from user ${userId}: ${error}`,
        );
        response['message'] = 'Internal server error.';
        response['error'] =
            'There was an error removing the active items for the given user.';
        res.status(500).json(response);
    }
}
...
export async function getActiveItemById(
    ...
    try {
        const activeItem = await ActiveItemService.retrieveActiveItemById(
            userId,
            groceryItemId,
        );
        ...
        if (activeItem) {
            response['message'] =
                'Successfully retrieved active item for user.';
        ...
        logger.error(
            `Error retrieving active item with id ${groceryItemId}: ${error}`,
        );
        response['message'] = 'Internal server error.';
        response['error'] =
            'There was an error retrieving the active item for the given user.';
        res.status(500).json(response);
    }
}
...
export async function getActiveItems(
    ...
    try {
        ...
        if (activeItems.length > 0) {
            response['message'] =
                'Successfully retrieved all active items for user.';
            ...
            response['error'] =
                'There was an error retrieving the active items for the given user.';
            res.status(404).json(response);
        }
    } catch (error) {
        logger.error(
            `Error retrieving active items for user with id ${userId}: ${error}`,
        );
        response['message'] = 'Internal server error.';
        response['error'] =
            'There was an error retrieving the active items for the given user.';
