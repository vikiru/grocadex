...
type ExpenseState = {
    ...
    getExpensesByMonthYear: (month: number, year: number) => Expense[] | undefined;
    ...
    addReceiptToExpenses: (expenseId: number, receiptId: number, receiptTotal: number) => void;
    ...
    removeReceiptFromExpenses: (expenseId: number, receiptId: number, receiptTotal: number) => void;
};
...
export const useExpenseStore = create<ExpenseState>()(
    persist(
        (set, get) => ({
            ...
            getExpenseFromId: (id: number) => get().expenses.find((expense) => expense.id === id),
            getExpensesByMonth: (month: number) => get().expenses.filter((expense) => expense.month === month),
            ...
            updateExpense: (expenseId: number, updatedExpense: Expense) => {
                set({
                    expenses: get().expenses.map((expense) => (expense.id === expenseId ? updatedExpense : expense)),
                });
            },
            resetExpenses: () => set({ expenses: [] }),
            getCurrentMonthlyExpenses: (month: number, year: number) => {
                return get()
                    .expenses.filter((expense) => expense.month === month && expense.year === year)
            ...
            addReceiptToExpenses: (expenseId: number, receiptId: number, receiptTotal: number) => {
                ...
                if (expense && !(expense.receiptIds ?? []).includes(receiptId)) {
                    ...
                    set({
                        expenses: get().expenses.map((exp) => (exp.id === expenseId ? updatedExpense : exp)),
                    });
                }
            },
            updateReceiptInExpenses: (
