...
axiosInstance.interceptors.response.use(
    ...
    async (error) => {
        ...
        if (statusCode === 401 && !originalRequest._retry) {
            ...
            if (refreshToken) {
                try {
                    const response: ResponsePayload = await axiosInstance.post(`${BASE_URL}/auth/refresh`, {
                    ...
                    return Promise.reject(new Error('Failed to refresh access token, please log in again'));
                }
            }
        }
        console.error(error);
